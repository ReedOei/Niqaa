...
gun :: Part.Part
gun = Part.Part (-1) (-1) (V2 0 0) (V2 0 0) 25 10 (-1) $ Part.Gun 5 2000 0

canShoot part@Part.Part{stats = Part.Gun _ goal timer} = timer > goal
canShoot _ = False

-- Should never be used on a non-gun, so let's make it fail if it is
resetTimer part@Part.Part{stats = Part.Gun prec goal _} = part {Part.stats = Part.Gun prec goal 0}
...
getFarthest U model ship = Safe.minimumBy (comparing (getY . Part.pos)) $ Map.elems $ getParts model ship
getFarthest D model ship = Safe.maximumBy (comparing (getY . Part.pos)) $ Map.elems $ getParts model ship
getFarthest L model ship = Safe.minimumBy (comparing (getX . Part.pos)) $ Map.elems $ getParts model ship
getFarthest R model ship = Safe.maximumBy (comparing (getX . Part.pos)) $ Map.elems $ getParts model ship
...
shoot model@(Model {..}) self@Part.Part{stats = Part.Gun prec _ _} targetPos
    ...
    where (miss, newGen) = randomR (-1, 1) gen 
          newShot = Shot.Shot nShots (Part.pos self) vel Shot.shot_size Shot.shot_damage (Part.factionId self) (Part.shipId self) 
            where vel = (fromAngle $ angle perfect + miss * prec) * pure Shot.shot_speed
